...
class ABTesting:
    ...
    def pooled_SE(self, Control, Exposed, X_A, X_B):
        ...
        SE = np.sqrt(p_hat * (1 - p_hat) * (1/Control + 1/Exposed)) #how is this computed?
        return SE
    ...
    def z_val(self, sig_level=0.05, two_tailed=True):
        ...
        z = z_dist.ppf(area) #ppf is the inverse of cdf: given the cumulative probablity (for all x<=x_0), it gives back x_0 
        # source: http://pytolearn.csd.auth.gr/d1-hyptest/11/distros.html
    ...
    def p_val(self, Control, Exposed, p_A, p_B):
        ...
        #Since P-value is defined as the probability of observing an outcome as extreme or more extreme than the one observed, 
        #assuming that the null hypothesis is true. The answer above is wrong.
        #Null is a normal distribution, mean 0 and std = pooled_SE()
        
        d_hat = p_B -p_A
        stderr = self.pooled_SE(Control, Exposed, p_A*Control, p_B* Exposed)
        dist = self.ab_dist(stderr, mde= d_hat, group_type='control')
        pvalue = (1 - dist.cdf(d_hat))*2
